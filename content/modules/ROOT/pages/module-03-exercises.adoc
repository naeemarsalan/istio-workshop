  ...
  default.conf: |
    server {
      ...
      location / {
        ...
        return 200 "app=loadtest version=v1 pod=\$hostname uri=\$request_uri\n";
      }
    }
---
apiVersion: apps/v1
...
Each pod reports its own name in the response, so you can count the distribution directly. Run the requests from a pod that is already in the mesh rather than starting one short-lived `oc run` pod per request:
...
oc exec -n $NAMESPACE $CLIENT -c test-container -- \
  sh -c 'for i in $(seq 1 30); do curl -s http://loadtest-service; done' \
  | grep -oE 'pod=[a-z0-9-]+' | sort | uniq -c
----

With `ROUND_ROBIN` across three pods the split is exact:

----
     10 pod=loadtest-service-8bb5b4478-9xpkn
     10 pod=loadtest-service-8bb5b4478-d9bbl
     10 pod=loadtest-service-8bb5b4478-dsm62
----

Repeat the same command after each load-balancer change below. `LEAST_CONN` and `RANDOM` give an uneven but roughly balanced spread (for example 11/7/12), because neither is strictly cyclic.

You can also confirm the requests in the access logs:
...
data:
  default.conf: |
    server {
      ...
      location / {
        default_type text/plain;
        return 200 "app=primary version=v1 pod=\$hostname uri=\$request_uri\n";
      }
    }
---
apiVersion: v1
...
data:
  default.conf: |
    server {
      ...
      location / {
        default_type text/plain;
        return 200 "app=backup version=v1 pod=\$hostname uri=\$request_uri\n";
      }
    }
---
apiVersion: apps/v1
...
NOTE: Both subsets above select `version: v1`, and both are defined on the host `primary-service`. That means the `backup` subset resolves to the *same* pods as `primary` — this DestinationRule demonstrates the outlier-detection *syntax*, but it does not actually fail traffic over to `backup-service`. Real failover across services needs either distinct labels on one host, or locality-based failover.
...
data:
  default.conf: |
    server {
      ...
      location / {
        default_type text/plain;
        return 200 "app=circuit pod=\$hostname uri=\$request_uri\n";
      }
    }
---
apiVersion: apps/v1
...
data:
  default.conf: |
    server {
      ...
      location / {
        default_type text/plain;
        return 200 "app=auth-service pod=\$hostname uri=\$request_uri\n";
      }
    }
---
apiVersion: apps/v1
...
IMPORTANT: A `RequestAuthentication` resource on its own does **not** require callers to present a token. It only says "if a token is present it must be valid". A request with *no* token is therefore still allowed through, and returns `200`. Only an `AuthorizationPolicy` (Exercise 5) can make a token mandatory. Expect:
...
[source,bash]
----
TOKEN=$(curl -s https://raw.githubusercontent.com/istio/istio/release-1.28/security/tools/jwt/samples/demo.jwt)
echo $TOKEN
----
...
IMPORTANT: If Kyverno is installed on your cluster, the short name `authorizationpolicy` is ambiguous — it also matches `authorizationpolicies.envoy.kyverno.io`, and `oc get`/`oc delete authorizationpolicy` will silently act on the wrong resource. Check with `oc api-resources | grep authorizationpolicies`. When in doubt use the fully-qualified name:

[source,bash]
----
oc get authorizationpolicies.security.istio.io -n $NAMESPACE
----
...
IMPORTANT: `AuthorizationPolicy` resources with `action: ALLOW` are *additive* — a request is permitted if **any** ALLOW policy matches it. The `require-jwt` policy you created in Step 1 allows every method for any authenticated caller, so it will keep permitting `DELETE` no matter what the policy below says, and the test in Step 4 will return `200` instead of `403`. Remove the broader policy first:

[source,bash]
----
oc delete authorizationpolicies.security.istio.io require-jwt -n $NAMESPACE
----

NOTE: Authorization changes take a few seconds to reach every sidecar. If a test returns the old result, wait ~30 seconds and try again before assuming the policy is wrong.
...
data:
  default.conf: |
    server {
      ...
      location / {
        default_type text/plain;
        return 200 "app=secure-service pod=\$hostname uri=\$request_uri\n";
      }
    }
---
apiVersion: apps/v1
