...
IMPORTANT: Two things about this module on OpenShift Service Mesh 3:

. *Access logging must be enabled.* It is off by default. Exercises 3 and 4 read Envoy access logs and will show nothing until you apply the namespace-scoped `Telemetry` resource from Module 1, Exercise 2, Step 6.
. *There is no `curl` inside the `istio-proxy` container.* The Red Hat proxy image ships a minimal root filesystem, so `oc exec ... -c istio-proxy -- curl ...` returns an empty result. Use `pilot-agent request GET <path>` instead — it queries the same Envoy admin endpoint. The exercises below already use `pilot-agent`. From your *workstation* (outside the pod), `curl` works normally against an `oc port-forward`.
...
oc exec -n $NAMESPACE $POD1 -c istio-proxy -- pilot-agent request GET stats/prometheus | grep istio_requests_total | head -5
----
...
oc exec -n $NAMESPACE $POD2 -c istio-proxy -- pilot-agent request GET stats/prometheus | grep istio_requests_total | head -5
----
...
for pod in $(oc get pods -n $NAMESPACE -l app=metrics-test -o jsonpath='{.items[*].metadata.name}'); do
  echo "Pod: $pod"
  oc exec -n $NAMESPACE $pod -c istio-proxy -- pilot-agent request GET stats/prometheus | grep "istio_requests_total.*destination_service_name=\"metrics-test\"" | head -3
done
----
...
NOTE: The default Envoy access-log format is a positional *text* line, not key/value pairs — there are no `duration="..."` or `request_size="..."` fields to grep for. A line looks like this:

----
[2026-09-18T13:51:12.485Z] "GET / HTTP/1.1" 200 - via_upstream - "-" 0 57 9 1 "-" "curl/8.22.0" ...
----

Reading the fields after the quoted request: `200` is the response code and `-` the response flags; after the upstream-failure field come `0` (bytes received), `57` (bytes sent), `9` (total duration in ms) and `1` (upstream service time in ms). The commands below extract those positional fields.
...
----
POD_NAME=$(oc get pod -n $NAMESPACE -l app=metrics-test -o jsonpath='{.items[0].metadata.name}')
oc logs -n $NAMESPACE $POD_NAME -c istio-proxy --tail=50 \
  | grep -oP '" [0-9]{3} \S+ \S+ \S+ "[^"]*" [0-9]+ [0-9]+ \K[0-9]+' | head -20
----
...
[source,bash]
----
oc logs -n $NAMESPACE $POD_NAME -c istio-proxy --tail=100 \
  | grep -oP '" \K[0-9]{3}' | sort | uniq -c
----
...
----
oc logs -n $NAMESPACE $POD_NAME -c istio-proxy --tail=100 \
  | grep -oP '" [0-9]{3} \S+ \S+ \S+ "[^"]*" [0-9]+ [0-9]+ \K[0-9]+' | sort -rn | head -5
----
...
oc logs -n $NAMESPACE $POD_NAME -c istio-proxy --tail=50 \
  | grep -oP '" [0-9]{3} \S+ \S+ \S+ "[^"]*" \K[0-9]+' | head -20

# bytes_sent (response size)
oc logs -n $NAMESPACE $POD_NAME -c istio-proxy --tail=50 \
  | grep -oP '" [0-9]{3} \S+ \S+ \S+ "[^"]*" [0-9]+ \K[0-9]+' | head -20
----
...
----
POD_NAME=$(oc get pod -n $NAMESPACE -l app=metrics-test -o jsonpath='{.items[0].metadata.name}')
istioctl proxy-config all $POD_NAME -n $NAMESPACE -o json | head -50
----
...
[source,bash]
----
istioctl experimental envoy-stats $POD_NAME -n $NAMESPACE
----
...
[source,bash]
----
istioctl experimental envoy-stats $POD_NAME -n $NAMESPACE | grep -i request
----
...
[source,bash]
----
istioctl experimental envoy-stats $POD_NAME -n $NAMESPACE | grep -i connection
----
...
for i in {1..5}; do
  ...
  oc exec -n $NAMESPACE $POD_NAME -c istio-proxy -- pilot-agent request GET stats/prometheus | grep "istio_requests_total.*destination_service_name=\"metrics-test\"" | head -3
  echo ""
  sleep 5
...
for i in {1..5}; do
  ...
  oc exec -n $NAMESPACE $POD_NAME -c istio-proxy -- pilot-agent request GET stats/prometheus | grep "istio_requests_total.*response_code=\"5" | head -3
  echo ""
  sleep 5
done
----
...
----
POD_NAME=$(oc get pod -n $NAMESPACE -l app=metrics-test -o jsonpath='{.items[0].metadata.name}')
BASELINE=$(oc exec -n $NAMESPACE $POD_NAME -c istio-proxy -- pilot-agent request GET stats/prometheus | grep "istio_requests_total.*destination_service_name=\"metrics-test\"" | head -1)
...
[source,bash]
----
AFTER=$(oc exec -n $NAMESPACE $POD_NAME -c istio-proxy -- pilot-agent request GET stats/prometheus | grep "istio_requests_total.*destination_service_name=\"metrics-test\"" | head -1)
